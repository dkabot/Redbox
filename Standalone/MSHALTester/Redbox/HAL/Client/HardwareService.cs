...
public sealed class HardwareService
{
    ...
    public HardwareCommandResult SyncUnknowns(HardwareJobSchedule schedule, out HardwareJob job)
    {
        ...
        job = hardwareCommandResult.Success ? HardwareJob.Parse(this, hardwareCommandResult.CommandMessages[0]) : null;
        return hardwareCommandResult;
    }
    ...
    public HardwareCommandResult HardSync(
        ...
        out HardwareJob job)
    {
        var hardwareCommandResult = ScheduleJob("sync-locations", label, false, schedule, out job);
        if (!hardwareCommandResult.Success)
            return hardwareCommandResult;
        PushLocs(locations, job);
        return hardwareCommandResult;
    ...
    public HardwareCommandResult HardSync(
        ...
        out HardwareJob job)
    {
        return HardSync(locations, null, schedule, out job);
    }
        ...
        out List<IHardwareCorrectionStatistic> stats)
    {
        ...
        using (var correctionStatistics = new GetHardwareCorrectionStatistics(this, stat))
        {
            correctionStatistics.Run();
            if (HardwareJobStatus.Completed == correctionStatistics.EndStatus)
                stats.AddRange(correctionStatistics.Stats);
    ...
    {
        stats = new List<IHardwareCorrectionStatistic>();
        using (var correctionStatistics = new GetAllHardwareCorrectionStatistics(this))
        {
            correctionStatistics.Run();
            if (HardwareJobStatus.Completed == correctionStatistics.EndStatus)
                stats.AddRange(correctionStatistics.Stats);
    ...
    public HardwareCommandResult RemoveHardwareCorrectionStats(
        ...
        out HardwareJob job)
    {
        var hardwareCommandResult = ScheduleJob("clear-hardware-statistics", null, false, schedule, out job);
        if (hardwareCommandResult.Success)
    ...
    public HardwareCommandResult RemoveHardwareCorrectionStats(
        ...
        out HardwareJob job)
    {
        return ScheduleJob("clear-all-hardware-statistics", null, false, schedule, out job);
    }
    ...
    {
        var inArray = CompressionAlgorithmFactory.GetAlgorithm(CompressionType.LZMA)
            .Compress(Encoding.ASCII.GetBytes(xmlConfig));
        return ExecuteCommand(
            string.Format("CONFIG set name: '{0}' data: '{1}'", name, Convert.ToBase64String(inArray)));
    }
    ...
    public HardwareCommandResult SetInventoryState(string xml)
    {
        return ExecuteCommand(string.Format("CONFIG set-inventory-state data: '{0}'",
            Convert.ToBase64String(CompressionAlgorithmFactory.GetAlgorithm(CompressionType.LZMA)
    ...
    {
        return ScheduleJob("power-cycle-router", s, out job);
    }
        ...
        out HardwareJob[] suspendedJobs)
    {
        ...
        foreach (var hardwareJob in jobs1)
            if ((!(hardwareJob.ID != Constants.ExecutionContexts.ImmediateModeContext) ? 0 :
                    ...
                    hardwareJob.Status == HardwareJobStatus.Running ? 1 : 0) != 0 && hardwareJob.Priority > priority)
            {
    ...
    public HardwareCommandResult CompileProgram(
        ...
        bool requiresClientConnection)
    {
        return ExecuteCommand(string.Format("PROGRAM compile path: @'{0}' name: '{1}' requires-client-connection: {2}",
            path, programName, requiresClientConnection));
    }
    ...
    {
        job = null;
        var hardwareCommandResult = ExecuteCommand(string.Format("JOB execute-immediate statement: '{0}'", statement),
    ...
    {
        return ScheduleJob("rebuild-inventory-database", s, out job);
    }

    public HardwareCommandResult TestAndResetCCR(HardwareJobSchedule schedule, out HardwareJob job)
    {
        return ScheduleJob("test-and-reset-ccr", null, false, schedule, out job);
    }
