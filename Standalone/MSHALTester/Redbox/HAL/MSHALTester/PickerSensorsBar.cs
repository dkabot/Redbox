...
public class PickerSensorsBar : UserControl
{
    ...
    private readonly Dictionary<int, Panel> m_sensors = new();
    ...
    public HardwareService Service { get; }
    ...
    {
        using (var buttonAspects = ButtonManager.MakeAspect(sender))
        {
            var tagInstruction = buttonAspects.GetTagInstruction();
            using (var instructionHelper = new InstructionHelper(Service))
            {
                var response = instructionHelper.ExecuteWithResponse(tagInstruction);
                if (BarEvents == null)
                    return;
                BarEvents(tagInstruction, response);
            }
        }
    }
    ...
    private void Read_Click(object sender, EventArgs e)
    {
        using (ButtonManager.MakeAspect(sender))
        {
            ...
            try
            {
                var result = ServiceLocator.Instance.GetService<IControlSystem>().ReadPickerSensors(false);
                readError = result.Error != 0;
                if (readError)
                    return;
                var i = 0;
                result.Foreach(each =>
                    m_sensors[i++].BackColor = result.IsInputActive(each) ? Color.Green : Color.WhiteSmoke);
            }
            finally
            {
                if (ReadEvents != null)
    ...
    private void InitializeComponent()
    {
        ...
        m_onButton.Tag = "CONTROLSYSTEM PICKERSENSORSON";
        m_onButton.Text = "On";
        ...
        m_offButton.Tag = "CONTROLSYSTEM PICKERSENSORSOFF";
