...
namespace Redbox.Shell.ComponentModel
{
    public class Error
    {
        ...
        public static Error NewError(string code, string description, Exception e)
        {
            ...
            if (e.InnerException != null)
                stringBuilder.AppendFormat("\r\n\r\nInner Exception:\r\n---------------------------\r\n{0}\r\n",
                    e.InnerException);
            stringBuilder.AppendFormat("\r\n\r\nStack Trace:\r\n----------------------------\r\n{0}\r\n", e.StackTrace);
            LogHelper.Instance.Log(string.Format("{0} - {1}", code, description), e, LogEntryType.Error);
        ...
        {
            LogHelper.Instance.Log(string.Format("{0} - {1} - {2}", code, description, details), LogEntryType.Error);
            return new Error(code, description, details, false);
        }

        public static Error NewError(string code, string description)
        {
            LogHelper.Instance.Log(string.Format("{0} - {1}", code, description), LogEntryType.Error);
