        ...
        foreach (XmlNode parentNode1 in xmlNodeList1)
            try
            {
                ...
                if (element != null)
                {
                    ...
                    {
                        var customAttribute =
                            (CustomEditorAttribute)Attribute.GetCustomAttribute(element, typeof(CustomEditorAttribute));
                        if (parentNode1.ChildNodes.Count > 0 && customAttribute != null &&
                            customAttribute.SetMethodName != null)
                        {
                            instance.GetType()
                                .GetMethod(customAttribute.SetMethodName, BindingFlags.Instance | BindingFlags.Public)
                        ...
                        else if (parentNode1.Attributes["value"] != null)
                        {
                            ...
                            element.SetValue(instance, ConversionHelper.ChangeType(str, element.PropertyType), null);
                        }
                    }
                }
            }
            ...
            if (customAttribute1 == null || customAttribute1.Browsable)
            {
                ...
                if (customAttribute6 != null)
                {
                    ...
                    if (customAttribute6.GetMethodName != null)
                        instance.GetType()
                            .GetMethod(customAttribute6.GetMethodName, BindingFlags.Instance | BindingFlags.Public)
                            ?.Invoke(instance, new object[1]
                            {
                                xmlWriter
                            });
                ...
                else
                {
                    ...
                    {
                        LogHelper.Instance.Log(string.Format("Walk tree: caught an exception {0}", ex.Message),
                            LogEntryType.Error);
                    }
