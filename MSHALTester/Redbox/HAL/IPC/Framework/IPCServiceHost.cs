...
public abstract class IPCServiceHost : IIpcServiceHost
{
    ...
    public void Unregister(ISession session)
    {
        ...
        if (!LogDetailedMessages)
            return;
        LogHelper.Instance.Log(string.Format("Unregister server session - there are {0} active server sessions.", num),
            LogEntryType.Debug);
    }
    ...
    public void Register(ISession session)
    {
        ...
        if (!LogDetailedMessages)
            return;
        LogHelper.Instance.Log(string.Format("Register server session - there are {0} active server sessions.", num),
            LogEntryType.Debug);
    }
