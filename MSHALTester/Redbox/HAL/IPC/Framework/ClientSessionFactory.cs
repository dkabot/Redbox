    ...
    public static IIpcClientSession GetClientSession(IPCProtocol protocol)
    {
        ...
        if (protocol.Channel == ChannelType.Socket)
        {
            ...
            LogHelper.Instance.Log(
                string.Format("TCP port specification {0} is incorrect; check your URI and try again.", protocol.Port),
                LogEntryType.Fatal);
            throw new Exception("Port specification in URI is incorrect.");
        }

        LogHelper.Instance.Log(LogEntryType.Error, "Unrecognized channel type {0}; please fix your protocol string.",
            protocol.Channel);
        return null;
    }
}