...
public sealed class HardwareJob
{
    ...
    public bool IsLocallyConnected => Connection != null;
    ...
    public HardwareCommandResult Connect()
    {
        ...
        session.ServerEvent += message =>
        {
            ...
            {
                var ignoringCase = Enum<HardwareJobStatus>.ParseIgnoringCase(
                    eventMessage.Substring(eventMessage.IndexOf("<") + 1).Trim(), HardwareJobStatus.Completed);
        ...
        var hardwareCommandResult =
            ClientCommand<HardwareCommandResult>.ExecuteCommand(session, string.Format("JOB connect job: '{0}'", ID));
        Connection = new ConnectionInfo(session);
        Connection.EventThread = new Thread(EventPoll)
        {
    ...
    public HardwareCommandResult GetResults(out ProgramResult[] results)
    {
        ...
        var list = new List<ProgramResult>();
        using (new DisposeableList<ProgramResult>(list))
        {
            if (results1.Success)
                results1.CommandMessages.ForEach(each =>
                {
                    var programResult = ProgramResult.FromString(each);
    ...
    public HardwareCommandResult Pop<T>(out T value)
    {
        ...
        if (hardwareCommandResult.Success)
            value = ConversionHelper.ChangeType<T>(hardwareCommandResult.CommandMessages[0]);
        return hardwareCommandResult;
    }
    ...
    {
        return ExecuteCommand(string.Format("DEBUG add-breakpoint job: '{0}' script: '{1}' line: {2}", ID, scriptName,
            lineNumber));
    }
    ...
    public bool Merge(HardwareJob job)
    {
        ...
        if (job.StartTime.HasValue)
        {
            ...
            if ((startTime1.HasValue == startTime2.HasValue
                    ? startTime1.HasValue ? startTime1.GetValueOrDefault() != startTime2.GetValueOrDefault() ? 1 : 0 : 0
                    : 1) != 0)
            {
    ...
    {
        using (var connectionInfo = o as ConnectionInfo)
        {
            do
            {
                if (!ClientCommand<HardwareCommandResult>.ExecuteCommand(connectionInfo.Session, "JOB scheduler-status")
                        .Success && EventRaised != null)
                    EventRaised(this, DateTime.Now, ">IPC EXCEPTION<");
            } while (!connectionInfo.WaitDisconnect());
        }

        ExecuteCommand(string.Format("JOB disconnect job: '{0}'", ID));
    }
    ...
    private class ConnectionInfo : IDisposable
    {
        ...
        internal ConnectionInfo(IIpcClientSession session)
        {
            Session = session;
        }

        internal IIpcClientSession Session { get; }
        ...
        internal bool WaitDisconnect()
        {
            return WaitDisconnect(250);
        }

        internal bool WaitDisconnect(int ms)
        {
            return ConnectEvent.WaitOne(ms);
        }
    }
}