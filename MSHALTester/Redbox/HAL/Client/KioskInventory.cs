...
public sealed class KioskInventory : IDisposable
{
    ...
    public List<IInventoryLocation> DeckInventory { get; } = new();

    public List<IDumpbinItem> DumpbinItems { get; } = new();
    ...
    private void Load()
    {
        ...
        using (var txtReader = new StringReader(inventoryState.CommandMessages[0]))
        {
            ...
            for (var i = 0; i < elementsByTagName1.Count; ++i)
            {
                ...
                DeckInventory.Add(new InventoryLocation(Convert.ToInt32(elementsByTagName1[i].Attributes["deck"].Value),
