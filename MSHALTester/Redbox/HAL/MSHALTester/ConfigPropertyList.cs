...
public class ConfigPropertyList : UserControl, IDisposable
{
    ...
    private readonly List<ConfigItem> m_cameraConfig = new();
    private readonly XmlDocument m_controller;
    private readonly List<ConfigItem> m_controllerConfig = new();
    ...
    {
        m_cameraConfig.FindAll(x => !x.ReadOnly && x.CustomEditor == null).ForEach(x => x.Value = x.DefaultValue);
        m_controllerConfig.FindAll(x => !x.ReadOnly && x.CustomEditor == null).ForEach(x => x.Value = x.DefaultValue);
        ListBoxSelectionChangedInner();
        IsDirty = false;
    ...
    private void m_filterTextBox_TextChanged(object sender, EventArgs e)
    {
        foreach (DataGridViewRow row in m_propertyDataGridView.Rows)
            if (string.IsNullOrEmpty(m_filterTextBox.Text))
                ...
                row.Visible = row.Cells[0].Value.ToString().ToLower().Contains(m_filterTextBox.Text.ToLower());
    }
    ...
    private void InitializeComponent()
    {
        ...
        m_propertyDataGridView.Anchor = AnchorStyles.Top | AnchorStyles.Bottom | AnchorStyles.Left | AnchorStyles.Right;
