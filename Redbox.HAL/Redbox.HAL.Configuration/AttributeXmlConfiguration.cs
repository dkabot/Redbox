        ...
        IConfigurationObserver
    {
        ...
        public void LoadProperties(XmlDocument document, ErrorList errors)
        {
            using (var executionTimer = new ExecutionTimer())
            {
                    ...
                    try
                    {
                        ...
                        if (property == null)
                        {
                            if (LogHelper.Instance.IsLevelEnabled(LogEntryType.Debug))
                                LogHelper.Instance.Log("[AttributeXmlConfiguration] Unable to find property '{0}'",
                                    name);
                        }
                ...
                LogHelper.Instance.Log("[AbstractXmlConfiguration] Time to load defaults on type {0}: {1}ms",
                    ThisType.Name, executionTimer.ElapsedMilliseconds);
                executionTimer.Stop();
            }
        ...
        public void StoreProperties(XmlDocument document, ErrorList errors)
        {
            using (var executionTimer = new ExecutionTimer())
            {
                ...
                executionTimer.Stop();
                LogHelper.Instance.Log("[AbstractXmlConfiguration] Time to save defaults on type {0}: {1}ms",
                    ThisType.Name, executionTimer.ElapsedMilliseconds);
            }
            ...
            try
            {
                ...
                if (xmlNode != null)
                    if (!string.IsNullOrEmpty(xmlNode.InnerText))
                        return (T)ConversionHelper.ChangeType(service.ExpandConstantMacros(xmlNode.InnerText),
                            typeof(T));
            }
