...
namespace Redbox.HAL.Controller.Framework
{
    public sealed class ControllerConfiguration : AttributeXmlConfiguration
    {
        ...
        {
            GearY = new MotorGear(20.0, 0, 0, 4);
            GearX = new MotorGear(10.0, 398, 114, 8);
            InitAxisXSpeed = new MotorSpeed(500, 3000, 300, GearX);
            InitAxisYSpeed = new MotorSpeed(600, 1500, 300, GearY);
            MoveAxisYSpeed = new MotorSpeed(500, 15000, 500, GearY);
            MoveAxisXSpeed = new MotorSpeed(500, 15000, 500, GearX);
            MoveAxisXYSpeed = new MotorSpeed(500, 15000, 500, GearY);
        }
        ...
        [DisplayName("Decks")]
        [CustomEditor("(Decks Collection)", GetMethodName = "ConvertDeckConfigurationToXml",
            SetMethodName = "UpdateDeckConfigurationFromXml")]
        [Description("The physical description of the decks for the host of the connected HAL Service.")]
        public int DecksCount => Manager.DeckCount;
        ...
        [Description(
            "The amount of time the gripper will extend the arm before timing out; used when putting a DVD into the slot.")]
        ...
        [Description(
            "The amount of time the gripper will overrun the limit switch beforing raising a timeout while testing for a DVD in the slot.")]
        ...
        [Description(
            "Controls how long (in milliseconds) HAL waits for the Arcus controller to fill its internal buffer before reading it.")]
        ...
        [Description(
            "Known as the vend position, this is the point on the Y axis that is equal to the vend door, allowing the picker to vend and return disks. This must be a negative integer value.")]
        ...
        [Description(
            "The amount of back off movement on the Y axis during a HOMEY instruction.  This value should be used only on older machines where the upper Y axis limit switch is too close to the Y axis home switch.")]
        ...
        [Description(
            "The X axis motor gear configuration.  The configuration of this property and its children affect the X Axis Speed properties.")]
        ...
        [Description(
            "The Y axis motor gear configuration.  The configuration of this property and its children affect the Y Axis Speed properties.")]
        ...
        [ValidValueListProvider("GetComPorts")]
        [Description(
            "The COM port the main, picker, auxiliary, and alternate motion controller boards are connected to.")]
        ...
        [Description(
            "If set to true, will time-extend the fingers using the QlmExtendTime, ignoring the extend sensor.")]
        ...
        [Description(
            "If set to true ( the default ), qlm/thin/qlm-thin will reboot when detecting an Arcus problem after engage/disengage.")]
        [XmlConfiguration(DefaultValue = "false")]
        ...
        [Description(
            "If true, query the current position before moving; if at the target position, wont' request a move.")]
        [XmlConfiguration(DefaultValue = "true")]
        ...
        [Description(
            "The amount of time the gripper will overrun the limit switch beforing raising a timeout while doing a grab on the QLM.")]
        [XmlConfiguration(DefaultValue = "1500")]
        ...
        [Description(
            "If set to true, will check both the forward and retract sensors if they're both set, and if they are, won't move.")]
        [XmlConfiguration(DefaultValue = "true")]
        ...
        [Description(
            "If set to true, will check if the gripper is in the rent position, and if not, will move it to Rent.")]
        [XmlConfiguration(DefaultValue = "true")]
        ...
        [Description(
            "If set to true, will do a slot test, and if the slot is full, will rotate the drum 5 slots during an unknown removal if unable to grab a disk.")]
        [XmlConfiguration(DefaultValue = "true")]
        ...
        [Description(
            "If set to true, will leave a CAMERA CAPTURE result during a return job if duplicates are detected.")]
        [XmlConfiguration(DefaultValue = "true")]
        ...
        [Description(
            "If greater than 0, will spin for the configured time in ms after clearing sensor 4 during a vend disk.")]
        [XmlConfiguration(DefaultValue = "50")]
        ...
        [Description(
            "If Empty or stuck count exceeds this number during vend in a 24 hour period, generate a result that should trigger an alert.")]
        [XmlConfiguration(DefaultValue = "15")]
        ...
        [Description(
            "If true, will leave a RESULT code for alert generation when the empty/stuck counter has exceeded its threshold.")]
        [XmlConfiguration(DefaultValue = "false")]
        ...
        [Description(
            "An integer describing how many consecutive failures happen before an alert is generated from return.")]
        [XmlConfiguration(DefaultValue = "2")]
        ...
        [Description(
            "If true, will retake image and scan for fraud markers.  Depends on EnableCameraReset to be set to true.")]
        [XmlConfiguration(DefaultValue = "true")]
        ...
        [Description(
            "The number of attempts to try and read a disc without fraud markers. will recenter in some scenarios.")]
        [XmlConfiguration(DefaultValue = "1")]
        ...
        [Browsable(false)] public bool IsVMZMachine => MerchMode != 0;
        ...
        internal int RegistrySlotCount
        {
            get
            {
                ...
                try
                {
                    ...
                    if (registryKey != null)
                    {
                        var obj = registryKey.GetValue("MaxSlots");
                        if (obj != null)
        ...
        {
            Manager.ToPropertyXml(writer);
        }

        public void UpdateDeckConfigurationFromXml(XmlNode propertyNode)
        {
            Manager.UpdateFromPropertyXml(propertyNode);
        }

        protected override void ImportInner(ErrorList errors)
        {
            var gampHelper = new GampHelper();
            var platterSlots = gampHelper.GetPlatterSlots();
            if (platterSlots == -1)
            {
                errors.Add(Error.NewError("H322", "The Slotdata.dat file was not found or had no entries.",
                    "Check the C:\\gamp folder to ensure the file exists."));
            }
            else
            {
                var registrySlotCount = RegistrySlotCount;
                if (platterSlots != registrySlotCount)
                {
                    errors.Add(Error.NewError("H323", "Configuration mismatch error.",
                        string.Format(
                            "The machine is configured for {0} slots; the gamp data says there are {1} slots.",
                            registrySlotCount, platterSlots)));
                }
                else
                {
                    var nullable1 = new int?();
                    var nullable2 = new int?();
                    var intList1 = new List<int>();
                    var boolList = new List<bool>();
                    var dictionary = gampHelper.ReadLegacySystemDataFile();
                    if (dictionary.Count == 0)
                    {
                        errors.Add(Error.NewError("H321", "The SystemData.dat file was not found or had no properties.",
                            "Check the C:\\gamp folder to ensure the file exists."));
                    }
                    else
                    {
                        if (dictionary.ContainsKey("PushTime"))
                            PushTime = dictionary["PushTime"];
                        if (dictionary.ContainsKey("RollInExtendTime"))
                            RollInExtendTime = dictionary["RollInExtendTime"];
                        if (dictionary.ContainsKey("TestExtendTime"))
                            TestExtendTime = dictionary["TestExtendTime"];
                        if (dictionary.ContainsKey("PickExtendTime"))
                            QlmExtendTime = dictionary["PickExtendTime"];
                        if (dictionary.ContainsKey("QLMaproch"))
                            QlmApproachOffset = dictionary["QLMaproch"];
                        if (dictionary.ContainsKey("QLMZoffset"))
                            QlmYOffset = dictionary["QLMZoffset"];
                        if (dictionary.ContainsKey("ReceivePositionOffSet"))
                            VendPositionReceiveOffset = dictionary["ReceivePositionOffSet"];
                        if (dictionary.ContainsKey("ArcusComPort"))
                            MotionControllerPortName = string.Format("COM{0}", dictionary["ArcusComPort"]);
                        if (dictionary.ContainsKey("ControlComPort"))
                            ControllerPortName = string.Format("COM{0}", dictionary["ControlComPort"]);
                        VendYPosition = dictionary.ContainsKey("VendPosition") ? dictionary["VendPosition"] : -86200;
                        if (dictionary.ContainsKey("ZHomeDropBack"))
                            HomeYDropBack = -dictionary["ZHomeDropBack"];
                        NumberOfPulls = dictionary.ContainsKey("NumberOfPulls") ? dictionary["NumberOfPulls"] : 2;
                        if (dictionary.ContainsKey("SellThrughOffSet"))
                            nullable2 = dictionary["SellThrughOffSet"];
                        if (dictionary.ContainsKey("QLMDeckNumber"))
                            nullable1 = dictionary["QLMDeckNumber"];
                        GearX = new MotorGear(dictionary.ContainsKey("RGearRatio") ? dictionary["RGearRatio"] : 10.0,
                            dictionary.ContainsKey("RPulseRatio") ? dictionary["RPulseRatio"] : 398,
                            dictionary.ContainsKey("REncoderRatio") ? dictionary["REncoderRatio"] : 114,
                            dictionary.ContainsKey("RStepRes") ? dictionary["RStepRes"] : 8);
                        GearY = new MotorGear(dictionary.ContainsKey("ZGearRatio") ? dictionary["ZGearRatio"] : 20.0, 0,
                            0, dictionary.ContainsKey("ZStepRes") ? dictionary["ZStepRes"] : 4);
                        InitAxisXSpeed =
                            new MotorSpeed(dictionary.ContainsKey("InitAxis1LS") ? dictionary["InitAxis1LS"] : 600,
                                dictionary.ContainsKey("InitAxis1HS") ? dictionary["InitAxis1HS"] : 3000, 300, GearX);
                        InitAxisYSpeed =
                            new MotorSpeed(dictionary.ContainsKey("InitAxis2LS") ? dictionary["InitAxis2LS"] : 800,
                                dictionary.ContainsKey("InitAxis2HS") ? dictionary["InitAxis2HS"] : 2400, 300, GearY);
                        MoveAxisYSpeed = new MotorSpeed(500,
                            dictionary.ContainsKey("MoveZHS") ? dictionary["MoveZHS"] : 15000,
                            dictionary.ContainsKey("MoveZAcc") ? dictionary["MoveZAcc"] : 500, GearY);
                        MoveAxisXSpeed = new MotorSpeed(500,
                            dictionary.ContainsKey("MoveRHS") ? dictionary["MoveRHS"] : 15000,
                            dictionary.ContainsKey("MoveRAcc") ? dictionary["MoveRAcc"] : 500, GearY);
                        MoveAxisXYSpeed = new MotorSpeed(500,
                            dictionary.ContainsKey("MoveZRHS") ? dictionary["MoveZRHS"] : 15000,
                            dictionary.ContainsKey("MoveZRAcc") ? dictionary["MoveZRAcc"] : 500, GearY);
                        var num1 = 1;
                        while (true)
                        {
                            var key = string.Format("PlatterMaxSlots{0}", num1++);
                            if (dictionary.ContainsKey(key))
                            {
                                var num2 = dictionary.ContainsKey(key) ? dictionary[key] : 90;
                                intList1.Add(num2);
                            }
                            else
                            {
                        ...
                        while (true)
                        {
                            var key = string.Format("DeckSellStat{0}", num3++);
                            if (dictionary.ContainsKey(key))
                        ...
                        var intListList = gampHelper.ReadLegacySlotDataFile(false);
                        Manager.Clear();
                        var num4 = 1;
                        var rangeArray = new Range[5]
                        {
                            new Range(1, 1),
                            new Range(1, 19),
                            new Range(20, 40),
                        ...
                        foreach (var intList2 in intListList)
                        {
                            var num5 = num4;
                            var nullable3 = nullable1;
                            var valueOrDefault = nullable3.GetValueOrDefault();
                            var num6 = (num5 == valueOrDefault) & nullable3.HasValue ? 80 : intList1[num4 - 1];
                            var quadrantList = new List<Quadrant>();
                            for (var index = 1; index < intList2.Count && intList2[index] != 0; ++index)
                                if (nullable1.HasValue && num4 == nullable1.Value && index < rangeArray.Length)
                                    quadrantList.Add(new Quadrant(intList2[index], rangeArray[index]));
                                else
                                    quadrantList.Add(new Quadrant(intList2[index]));
                            var num7 = 166.6667M;
                            if (nullable1.HasValue && num4 == nullable1.Value)
                                num7 = 177.7M;
                            var flag = nullable1.HasValue && num4 == nullable1.Value;
                            var manager = Manager;
                            var number = num4;
                            var yoffset = intList2[0];
                            var num8 = flag ? 1 : 0;
                            var numberOfSlots = num6;
                            var slotWidth = num6 == 72 ? 173.3M : num7;
                            int? sellThruSlots;
                            if (num6 != 72)
                            {
                                nullable3 = new int?();
                            ...
                            var array = quadrantList.ToArray();
                            var deck = new Deck(number, yoffset, num8 != 0, numberOfSlots, slotWidth, sellThruSlots,
                                sellThruOffset, array);
                            manager.Add(deck);
                            ++num4;
                        }
                    }
        ...
        {
            document.DocumentElement.SelectSingleNodeAndSetValue("Controller/AggressiveClearPickerOnPut", false);
            document.DocumentElement.SelectSingleNodeAndSetValue("Controller/RestartControllerDuringUserJobs", true);
            document.DocumentElement.SelectSingleNodeAndSetValue("Controller/TrackHardwareCorrections", true);
            document.DocumentElement.SelectSingleNodeAndSetValue("Controller/PrepositionVendFraudMove", true);
        }

        protected override void StorePropertiesInner(XmlDocument document, ErrorList errors)
        {
            GearX.SaveToXml(document.DocumentElement.SelectSingleNode("Controller/GearX"));
            GearY.SaveToXml(document.DocumentElement.SelectSingleNode("Controller/GearY"));
            InitAxisXSpeed.SaveToXml(document.DocumentElement.SelectSingleNode("Controller/InitAxisXSpeed"));
            InitAxisYSpeed.SaveToXml(document.DocumentElement.SelectSingleNode("Controller/InitAxisYSpeed"));
            MoveAxisXSpeed.SaveToXml(document.DocumentElement.SelectSingleNode("Controller/MoveXAxisSpeed"));
            MoveAxisYSpeed.SaveToXml(document.DocumentElement.SelectSingleNode("Controller/MoveYAxisSpeed"));
            MoveAxisXYSpeed.SaveToXml(document.DocumentElement.SelectSingleNode("Controller/MoveXYAxisSpeed"));
            Manager.SaveConfiguration(document);
        }

        protected override void LoadPropertiesInner(XmlDocument document, ErrorList errors)
        {
            var documentElement = document.DocumentElement;
            var node1 = document.DocumentElement.SelectSingleNode("Controller/GearX");
            if (node1 == null)
            {
                errors.Add(Error.NewError("P002", "The required GearX node does not exist.",
                    "Add a valid GearX node to the configuration file."));
            }
            else
            {
                GearX = MotorGear.FromXmlNode(node1);
                var node2 = document.DocumentElement.SelectSingleNode("Controller/GearY");
                if (node2 == null)
                {
                    errors.Add(Error.NewError("P002", "The required GearY node does not exist.",
                        "Add a valid GearY node to the configuration file."));
                }
                else
                {
                    GearY = MotorGear.FromXmlNode(node2);
                    var node3 = document.DocumentElement.SelectSingleNode("Controller/InitAxisXSpeed");
                    if (node3 == null)
                    {
                        errors.Add(Error.NewError("P003", "The required InitAxisXSpeed node does not exist.",
                            "Add a valid InitAxisXSpeed node to the configuration file."));
                    }
                    else
                    {
                        InitAxisXSpeed = MotorSpeed.FromXmlNode(node3, GearX);
                        var node4 = document.DocumentElement.SelectSingleNode("Controller/InitAxisYSpeed");
                        if (node4 == null)
                        {
                            errors.Add(Error.NewError("P003", "The required InitYAxisSpeed node does not exist.",
                                "Add a valid InitYAxisSpeed node to the configuration file."));
                        }
                        else
                        {
                            InitAxisYSpeed = MotorSpeed.FromXmlNode(node4, GearY);
                            var node5 = document.DocumentElement.SelectSingleNode("Controller/MoveXAxisSpeed");
                            if (node5 == null)
                            {
                                errors.Add(Error.NewError("P004", "The required MoveXAxisSpeed node does not exist.",
                                    "Add a valid MoveXAxisSpeed node to the configuration file."));
                            }
                            else
                            {
                                MoveAxisXSpeed = MotorSpeed.FromXmlNode(node5, GearX);
                                var node6 = document.DocumentElement.SelectSingleNode("Controller/MoveYAxisSpeed");
                                if (node6 == null)
                                {
                                    errors.Add(Error.NewError("P004",
                                        "The required MoveYAxisSpeed node does not exist.",
                                        "Add a valid MoveYAxisSpeed node to the configuration file."));
                                }
                                else
                                {
                                    MoveAxisYSpeed = MotorSpeed.FromXmlNode(node6, GearY);
                                    var node7 = document.DocumentElement.SelectSingleNode("Controller/MoveXYAxisSpeed");
                                    if (node7 == null)
                                    {
                                        errors.Add(Error.NewError("P004",
                                            "The required MoveXYAxisSpeed node does not exist.",
                                            "Add a valid MoveXYAxisSpeed node to the configuration file."));
                                    }
                                    else
                                    {
                                        MoveAxisXYSpeed = MotorSpeed.FromXmlNode(node7, GearY);
                                        var decksNode = document.DocumentElement.SelectSingleNode("Controller/Decks");
                                        if (decksNode == null)
                                        {
                                            LogHelper.Instance.Log(
                                                "There are no decks nodes in the configuration file.");
                                            errors.Add(Error.NewError("P006", "No configured decks.",
                                                "There are no decks nodes in the configuration file."));
                                        }
                                        else
                                        {
                                            Manager.Initialize(decksNode);
                                            var registrySlotCount = RegistrySlotCount;
                                            var first = Manager.First;
                                            if (registrySlotCount != first.NumberOfSlots)
                                                errors.Add(Error.NewError("P005", "Machine configuration mismatch.",
                                                    string.Format("The store key shows {0}, but the deck shows {1}",
                                                        registrySlotCount, first.NumberOfSlots)));
                                            MerchMode = Manager.QlmDeck == null
                                                ? MerchandizingMode.VMZ_Gen2
                                                : MerchandizingMode.QLM;
                                            LogHelper.Instance.Log("Kiosk is configured for merchandizing mode {0}",
                                                MerchMode.ToString().ToUpper());
                                        }
                                    }
                                }
                            }
        ...
        internal void Initialize()
        {
            ServiceLocator.Instance.GetService<IConfigurationService>()
                .RegisterConfiguration(Configurations.Controller.ToString(), this);
            ServiceLocator.Instance.AddService<IDecksService>(Manager);
        }
    }
}