...
namespace Redbox.DirectShow
{
    public class DirectShowFrameControl_v2 : IVideoSource, ISampleGrabberCB
    {
        ...
        public unsafe int BufferCB(double sampleTime, IntPtr buffer, int bufferLen)
        {
            ...
            if (Debug)
            {
                LogHelper.Instance.Log("[{0}]: BufferCB ( request = {1} ) for file {2}.", "DirectShowSlim_v2",
                    m_request, bitmapToSave);
                if (File.Exists(bitmapToSave))
                    LogHelper.Instance.Log("{0} WARNING: the file {1} already exists.", "DirectShowSlim_v2",
                        bitmapToSave);
            }
        ...
        public bool Start()
        {
            ...
            if (string.IsNullOrEmpty(deviceMoniker))
            {
                LogHelper.Instance.Log("[{0}] Video source is not specified - cannot start graph.",
                    "DirectShowSlim_v2");
                return false;
            }
