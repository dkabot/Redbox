...
namespace Redbox.DirectShow
{
    public class PlayerDevice : IVideoSource
    {
        ...
        private static readonly Dictionary<string, VideoCapabilities[]> cacheSnapshotCapabilities =
            new Dictionary<string, VideoCapabilities[]>();
        ...
        private void WorkerThread(bool runGraph)
        {
            ...
            try
            {
                ...
                var baseFilter3 = SourceObject != null
                    ? (IBaseFilter)SourceObject
                    : throw new ApplicationException("Failed creating device object for moniker");
                try
                {
                    var sourceObject = (IAMVideoControl)SourceObject;
                }
                catch
                {
                }
                ...
                graphBuilder.FindInterface(FindDirection.UpstreamOnly, Guid.Empty, baseFilter3,
                ...
                GetPinCapabilitiesAndConfigureSizeAndRate(graphBuilder, baseFilter3, PinCategory.Capture,
                        ...
                        if (m_simulateTrigger)
                        {
                            ...
                            lock (m_listLock)
                            {
                                if (ReceivedFrames.Count > 0 && SnapshotFrame != null)
                                    SnapshotFrame(this,
                                        new NewFrameEventArgs(ReceivedFrames[ReceivedFrames.Count - 1]));
                                else
                                    LogHelper.Instance.Log(
                                        "[DirectShow] There are no captured frames from the device.");
