...
namespace Redbox.NetCore.Logging.Extensions
{
    public static class SerilogExtensions
    {
        ...
        private static void CheckForMinimumRequiredSerilogConfiguration(this IConfiguration configuration)
        {
            ...
            if (configuration.GetSection("Serilog:Using").GetChildren()
                .DoesNotExist(value => value.Value == "Serilog.Sinks.Console"))
                throw new ConfigurationErrorsException(
                    "'Serilog:Using' configuration file section is missing Serilog.Sinks.Console value!");
            ...
            if (configuration.GetSection("Serilog:MinimumLevel:Default").DoesNotExist())
                throw new ConfigurationErrorsException(
                    "'Serilog:MinimumLevel:Default' configuration file value is missing!");
            if (configuration.GetSection("Serilog:MinimumLevel:Override").DoesNotExist())
                throw new ConfigurationErrorsException(
                    "'Serilog:MinimumLevel:Override' configuration file section is missing!");
            if (configuration.GetSection("Serilog:MinimumLevel:Override:System").DoesNotExist())
                throw new ConfigurationErrorsException(
                    "'Serilog:MinimumLevel:Override:System' configuration file value is missing!");
            if (configuration.GetSection("Serilog:MinimumLevel:Override:Microsoft").DoesNotExist())
                throw new ConfigurationErrorsException(
                    "'Serilog:MinimumLevel:Override:Microsoft' configuration file value is missing!");
            if (configuration.GetSection("Serilog:MinimumLevel:Override:Microsoft.AspNetCore.Mvc.Internal")
                .DoesNotExist())
                throw new ConfigurationErrorsException(
                    "'Serilog:MinimumLevel:Override:Microsoft.AspNetCore.Mvc.Internal' configuration file value is missing!");
            ...
            if (configuration.GetSection("Serilog:Enrich").GetChildren()
                .DoesNotExist(value => value.Value == "FromLogContext"))
                throw new ConfigurationErrorsException(
                    "'Serilog:Enrich' configuration file section is missing FromLogContext value!");
            if (configuration.GetSection("Serilog:Enrich").GetChildren()
                .DoesNotExist(value => value.Value == "WithThreadId"))
                throw new ConfigurationErrorsException(
                    "'Serilog:Enrich' configuration file section is missing WithThreadId value!");
            ...
            if (configuration.GetSection("Serilog:WriteTo:0").DoesNotExist())
                throw new ConfigurationErrorsException(
                    "'Serilog:WriteTo' configuration file section is missing first section!");
            if (configuration.GetSection("Serilog:WriteTo:0:Name").DoesNotExist())
                throw new ConfigurationErrorsException(
                    "'Serilog:WriteTo' configuration file first section is missing Name value!");
            if (configuration.GetSection("Serilog:WriteTo:0:Name").Value != "Async")
                throw new ConfigurationErrorsException(
                    "'Serilog:WriteTo' configuration file first section is not the Async section!");
        }

        private static bool DoesNotExist(this IConfigurationSection section)
        {
            return !section.Exists();
        }

        private static bool DoesNotExist(this IEnumerable<IConfigurationSection> enumerable,
            Predicate<IConfigurationSection> match)
        {
            return !enumerable.ToList().Exists(match);
        }
    }
}