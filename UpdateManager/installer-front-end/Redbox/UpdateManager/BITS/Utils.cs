...
namespace Redbox.UpdateManager.BITS
{
    ...
    {
        private static BitsVersion version = Utils.GetBitsVersion();

        internal static string GetName(string SID)
        {
            long maxValue1 = byte.MaxValue;
            long maxValue2 = byte.MaxValue;
            IntPtr SID1 = new IntPtr(0);
            int psUse = 0;
            StringBuilder name = new StringBuilder(byte.MaxValue);
            StringBuilder domainName = new StringBuilder(byte.MaxValue);
        ...
        private static BitsVersion GetBitsVersion()
        {
            try
            {
                string sFileName = Path.Combine(System.Environment.SystemDirectory, "qmgr.dll");
                int handle = 0;
