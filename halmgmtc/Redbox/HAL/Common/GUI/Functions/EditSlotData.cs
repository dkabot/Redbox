...
namespace Redbox.HAL.Common.GUI.Functions
{
    public class EditSlotData : Form
    {
        ...
        {
            InitializeComponent();
            OutputBox = new ESDOutput(m_OutputListBox);
            Service = service;
            Manager = mgr;
        ...
        private int CurrentEY { get; set; }

        private int CurrentEX { get; set; }

        private int OriginalEY { get; set; }

        private int OriginalEX { get; set; }

        private bool PositionsRecorded { get; set; }

        private bool MoveFailed { get; set; }

        private DecksConfigurationManager Manager { get; }
        ...
        private bool moveEncoderUnits(Direction direction)
        {
            ...
            if (int.TryParse(text, out result))
            {
                ...
                var moveHelperResult = moveHelper.MoveAbs(flag ? MoveAxis.Y : MoveAxis.X, units);
                OutputBox.Write("MOVE {0} {1} UNITS - New Location {2} ...", direction.ToString().ToUpper(), text,
                    units);
                if (!moveHelperResult.MoveOk)
                {
                    OutputBox.Write(moveHelperResult.Status);
                    return false;
                }
                ...
                if (flag)
                    ...
                    CurrentEX = units;
                OutputBox.Write(moveHelperResult.Status);
                var position1 = new MotorPosition(MoveAxis.X);
                var position2 = new MotorPosition(MoveAxis.Y);
                moveHelper.GetPosition(ref position1);
                moveHelper.GetPosition(ref position2);
        ...
        private void SavePositionalState()
        {
            var num = 0;
            var position1 = new MotorPosition(MoveAxis.Y);
            var moveHelper = new MoveHelper(Service);
            moveHelper.GetPosition(ref position1);
            if (position1.ReadOk)
            {
                OriginalEY = CurrentEY = position1.Position;
                ++num;
            }

            var position2 = new MotorPosition(MoveAxis.X);
            moveHelper.GetPosition(ref position2);
            if (position2.ReadOk)
            {
                OriginalEX = CurrentEX = position2.Position;
                ++num;
            }

            PositionsRecorded = num == 2;
        }
        ...
        private void MoveToStart(int deck, int slot)
        {
            try
            {
                var moveHelperResult = new MoveHelper(Service).MoveTo(deck, slot, true);
                if (moveHelperResult.MoveOk)
                {
                    m_moveStatusTB.Text = moveHelperResult.Status;
                    SavePositionalState();
                ...
                {
                    var stringBuilder = new StringBuilder();
                    stringBuilder.Append(moveHelperResult.Status);
                    foreach (var limit in moveHelperResult.Limits)
                        if (limit.Contains("BLOCKED"))
                        {
        ...
        private void RunSimpleCommand(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                HardwareJob job;
                if (!Service.ExecuteImmediate(tag, out job).Success)
                {
                    NotifyCommunicationProblem();
                }
                else
        ...
        private class ESDOutput : OutputBox
        {
            internal ESDOutput(ListBox box)
                : base(box)
            {
            }
        }
