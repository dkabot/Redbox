...
namespace UpdateClientService.API.Services.DownloadService
{
    public class DownloadData : IPersistentData
    {
        ...
        public string FileName => Key + DownloadDataConstants.DownloadExtension + ".json";
        ...
        {
            return string.Format("{0},{1},{2}-{3}", 1, revisionChangeSetKey.FileSetId, revisionChangeSetKey.RevisionId,
                revisionChangeSetKey.PatchRevisionId);
        }

        public long ParseFileSetIdFromDownloadDataKey()
        {
            long fromDownloadDataKey = 0;
            var source = Key.Split(new string[1] { "," }, StringSplitOptions.RemoveEmptyEntries);
            if (source.Count() > 1)
